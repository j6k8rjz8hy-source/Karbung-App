...
var SHEET_ID = '';
// Nama tab data. Bila tidak ditemukan, dipakai tab PERTAMA di spreadsheet.
  ...
  try{
    ...
    if(last >= 2){
      ...
      for(var i = 0; i < vals.length; i++){
        var r = vals[i];
        var pengirim = String(r[2]||'').trim();
        if(!pengirim || pengirim.toLowerCase() === 'pengirim') continue;
        var cid = String(r[7]||'');
        if(!cid){
          // Baris ketikan manual di sheet: beri ID agar ikut tampil di app.
          cid = Utilities.getUuid();
          sh.getRange(i + 2, 8).setValue(cid);
        }
        items.push({ tanggal:toIso_(r[0]), jenis:String(r[1]||''), pengirim:pengirim,
          tujuan:String(r[3]||''), grouping:String(r[4]||''), catatan:String(r[5]||''),
          fotoUrl:String(r[6]||''), cid:cid });
      }
    }
...
function getSheet_(){
  var ss = getSpreadsheet_();
  var sh = ss.getSheetByName(SHEET_NAME) || ss.getSheets()[0] || ss.insertSheet(SHEET_NAME);
  if(sh.getLastRow() === 0){
